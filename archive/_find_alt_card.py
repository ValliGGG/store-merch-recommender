"""Find the snippet/section rendering 'Alternatywne produkty' with double buttons in PL theme."""
import os, sys, json
...
print(f"theme: {main['id']} '{main['name']}'")
assets = json.loads(request.urlopen(request.Request(f"{api}/themes/{main['id']}/assets.json", headers=H)).read())["assets"]

TERMS = ["Alternatywne", "alternative_products", "Alternativne", "alternativne", "alternatives",
         "DO KOSZYKA", "Dodaj do koszyka", "frequently-bought", "alternative-product"]
for a in assets:
    key = a["key"]
    if not (key.endswith(".liquid") or key.endswith(".json") or key.endswith(".js")): continue
