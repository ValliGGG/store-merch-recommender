"""Find what's adding the second 'DO KOSZYKA' (icon+text) button on product cards."""
import os, sys, json
...
# Search for snippets that render a card with cart icon + uppercase text
TERMS = [
    "DO KOSZYKA", "DO KOŠÍKA", "В КОШНИЦА", "U KO\u0160ARICU",
    "cart-icon", "cart_icon", "cart-add", "card-add-to-cart", "card__add-to-cart",
    "add_to_cart_button", "product-card__cta",
    "artmie-card", "custom-product-card",
]

for a in assets:
    key = a["key"]
    if not (key.endswith(".liquid") or key.endswith(".json") or key.endswith(".js") or key.endswith(".css")):
        continue
